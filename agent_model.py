...
if __name__ == '__main__':
    print("Testing GOLKeyAgent initialization...")
    agent_cpu = None
    agent_gpu = None
    try:
        print("\n--- Testing on CPU ---")
        agent_cpu = GOLKeyAgent(device="cpu")
        print("Agent initialized successfully on CPU.")
        dummy_images_uint8_cpu = torch.randint(0, 256, (2, 3, 224, 224), dtype=torch.uint8)
        dummy_images_float_cpu = dummy_images_uint8_cpu.float() / 255.0

        print("\nTesting embed method (CPU)...")
        embeddings_cpu = agent_cpu.embed(dummy_images_float_cpu, max_batch=1)
        print(f"Embeddings shape: {embeddings_cpu.shape}, Dtype: {embeddings_cpu.dtype}")
        if embeddings_cpu.numel() > 0: print(f"Embeddings stats: mean={embeddings_cpu.mean().item():.4f}, std={embeddings_cpu.std().item():.4f}")

        print("\nTesting guess_word method (CPU)...")
        guess_cpu = agent_cpu.guess_word(dummy_images_float_cpu[0])
        print(f"Guessed word (CPU): '{guess_cpu}'")

        if torch.cuda.is_available():
            print("\n--- Testing on CUDA ---")
            agent_gpu = GOLKeyAgent(device="cuda")
            print("Agent initialized successfully on CUDA.")
            dummy_images_float_gpu = dummy_images_float_cpu.to(agent_gpu.device)

            print("\nTesting embed method (CUDA)...")
            embeddings_gpu = agent_gpu.embed(dummy_images_float_gpu, max_batch=1)
            print(f"Embeddings shape: {embeddings_gpu.shape}, Dtype: {embeddings_gpu.dtype}")
            if embeddings_gpu.numel() > 0: print(f"Embeddings stats: mean={embeddings_gpu.mean().item():.4f}, std={embeddings_gpu.std().item():.4f}")

            print("\nTesting guess_word method (CUDA)...")
            guess_gpu = agent_gpu.guess_word(dummy_images_float_gpu[0])
            print(f"Guessed word (CUDA): '{guess_gpu}'")
        else:
            print("\nCUDA not available, skipping GPU tests.")

        print("\n[GOLKeyAgent Test Completed]")

    except Exception as main_e:
        print(f"\n--- ERROR during GOLKeyAgent test ---")
        traceback.print_exc()
        print("-------------------------------------\n")
    finally:
         if agent_cpu and hasattr(agent_cpu, 'model'): del agent_cpu.model; del agent_cpu
         if agent_gpu and hasattr(agent_gpu, 'model'): del agent_gpu.model; del agent_gpu
         if torch.cuda.is_available(): torch.cuda.empty_cache()
         print("Cleaned up models and CUDA cache (if applicable).")